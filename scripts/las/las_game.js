...
class LasGame {
    ...
    recordCannonBallPositions(){
        ...
        for (let [cannonBall, cannonBallIndex] of this.cannonBalls){
            this.cannonBallPositions.push({"x_pos": cannonBall.getTickX(), "y_pos": cannonBall.getY()});
        }
    }
    ...
    handleCannonBallCollisionsAndDeaths(){
        ...
        for (let [cannonBall, cannonBallIndex] of this.cannonBalls){
            ...
            for (let [ship, shipIndex] of this.ships){
                ...
                let collisionDetails = checkCannonBallCollision(cannonBallPosData["x_pos"], cannonBallPosData["y_pos"], cannonBall.getTickX(), cannonBall.getTickY(), shipPosData["x_pos"], shipPosData["y_pos"], ship.getWidth(), ship.getHeight());
                // TODO
            }
        }
    }
    ...
    handleNewCannonShots(){
        ...
        for (let [cannonShotObj, index] of newCannonShots){
            ...
            cannonShotObj["death_tick"] = this.getTickCount() + cannonBallSettings["tick_life_length"];
    ...
    checkCannonBallCollision(cannonBallStartX, cannonBallStartY, cannonBallEndX, cannonBallEndY, shipStartX, shipStartY, shipEndX, shipEndY, shipWidth, shipHeight){
        ...
        let startsWithAHit = rectangleCollidesWithRectangle(shipStartX-(shipWidth-1)/2, shipStartY+(shipHeight-1)/2, shipStartX+(shipWidth-1)/2, shipStartY-(shipHeight-1)/2, cannonBallStartX-(cannonBallWidth-1)/2, cannonBallStartY+(cannonBallHeight-1)/2, cannonBallStartX+(cannonBallWidth-1)/2, cannonBallStartY-(cannonBallHeight-1)/2);
        if (startsWithAHit){
            return true;
        }
        ...
        // Left-right
        {
            ...
            let time = safeDivide(leftObjectRightEnd - rightObjectLeftEnd, rightDetails["v_x"] - leftDetails["v_x"], 0.0000001, null);
        ...
        // Top bottom
        {
            ...
            let time = safeDivide(bottomObjectTopEnd - topObjectBottomEnd, topDetails["v_y"] - bottomDetails["v_y"], 0.0000001, null);
            /* Expected values for time:
                null - Denominator close to zero
                < 0 - Never collide in x
                > 0 <= timeProportion - Collide in x at a reasonable time
            ...
            */
            ...
            if (time != null && time >= 0 && time <= timeProportion){
                ...
                let overlapInWidth = checkIfIntervalsOverlap(bottomObjectX + (bottomObject["width"]-1)/2, bottomObjectX + (bottomObject["width"]-1)/2, topObjectX + (topObject["width"]-1)/2, topObjectX + (topObject["width"]-1)/2);
                if (overlapInWidth){
